...
def get_next_result(lines, start):
    ...
    result['main_page'] = lines[start +
        abstract_line][abstract_begin_offset:abstract_begin_offset + idx]
    idx = lines[start + pdf_line][pdf_begin_offset:].find('"')
    result['pdf'] = lines[start +
        pdf_line][pdf_begin_offset: pdf_begin_offset + idx] + '.pdf'
...
def is_list_numer(tokens, i, value):
    ...
    if (i == 0 or tokens[i - 1] in list(['(', '.', ':'])) and tokens[i + 1] == ')':
        return True
    return False
...
def get_report(paper, keyword):
    if keyword in paper['abstract'].lower():
        ...
        headline = '{} ({} - {})'.format(title,
                        paper['authors'][0], paper['date'])
        abstract = h.unescape(paper['abstract'])
        extract, has_number = extract_line(
            abstract, keyword, 280 - len(headline))
        if extract:
            # report = {
            #     "headline": headline,
            #     "extract": extract,
...
def get_papers(keyword, num_results=5):
    ...
    while num_to_show > 0:
        ...
        #
        # txt = response.text
        req = urllib.request.Request(query, headers=ua_headers)
        ...
        if not found and not all_unshown and num_to_show == num_results:
            print(
                'Sorry, we were unable to find any abstract with the word {}'.format(keyword))
            return
...
def sota(keyword, num_results=5):
    if not isinstance(keyword, str):
        raise ValueError("It must be a string")

    if not isinstance(num_results, int):
        raise ValueError("It must be a int")
...
def replace_html(audio_lists, video_list):
    temp = '<tr>\
                ...
                <td class="column3 text-justify">{}</td>\
                <td class="column3 text-center">{}</td>\
            </tr>'
    tr = ''
    for detial in audio_lists:
        author = ''
        for a in range(len(detial['author'])):
            if a != len(detial['author'])-1:
